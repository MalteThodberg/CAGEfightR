...
bw_plus <- system.file("extdata", exampleDesign$BigWigPlus, package = "CAGEtestR")
bw_minus <- system.file("extdata", exampleDesign$BigWigMinus, package = "CAGEtestR")
...
# Quantify CTSSs
CTSSs <- quantifyCTSSs(plusStrand=bw_plus, minusStrand=bw_minus, design=exampleDesign, genome=genomeInfo)
```
...
Similarly, `quickEnhancers` will find and quantify enhancers:
...
Usually, enhancers overlapping known transcripts are removed:

```{r quickFilter, tidy=TRUE}
enhancers <- subset(enhancers, txType %in% c("intergenic", "intron"))
```

For usage with other Bioconductor package, it can be useful to merge TSSs and enhancers into a single object:
...
The following section contains a detailed walkthrough of most of the functions in `CAGEfightR`, using the built-in dataset. To keep this vignette compact, the example dataset is extremely small - for a more realistic analysis of a full CAGE dataset using using both `CAGEfightR` and additional packages, see the accompanying workflows _XXX_.

## CAGE Transcription Start Site (CTSS) level analysis

As shown in the introduction, the first step of a `CAGEfightR` analysis is import CTSSs from BigWig-files with the `quantifyCTSSs` function. Rather then repeating this, we use the built-in `exampleCTSSs` object:
...
`calcTPM` will first calculate the total number of tags for each sample and store them in colData and then scale counts into TPM and add them as a new assay:
...
Then `calcPooled` will sum TPM across sample for each CTSS and store it in the score-column:
...
In some cases you might have a very large number and/or very noisy samples (For example due to poor RNA quality), which can lead to an increase in the number of single tags spread across the genome. To alleviate this issue, CTSSd appearing in only a single or few samples can be discarded. We refer to this as calculating the _support_: the number of samples expressing a feature above some threshold:
...
### Unidirectional clustering: Finding Tag Cluster (TCs) or Transcription Start Sites (TSSs)

Once we have obtained pooled CTSSs, we can identify various types clusters: Transcripts are rarely transcribed from just a single CTSS, but rather from a collection or cluster of nearby CTSSs corresponding to a single TSS.

The simplest way of doing this is to group nearby CTSSs on the same strand into Tag Clusters (TCs), which are likely to represent real TSSs (although some post-filtering is usually a good idea, see next section). `CAGEfightR` does this by a slice-reduce approach: It finds sites above some value (slice) and then merges nearby sites (reduce). In the simplest form this can be run as:

```{r naiveTC, tidy=TRUE}
naive_TCs <- clusterUnidirectionally(exampleCTSSs, pooledCutoff=0, mergeDist=20)
```

`clusterUnidirectionally` find TCs and outputs some summary statistics. We see that we have some very wide TCs (> 1000 bp wide). This is the result of a few spread out tags that link several major TCs, so does represent the TSS structure very well.As explained in the `CAGEfightR` paper, this can be avoided by _tuning_ the cutoff used for slicing: The smallest value for cutoff value producing the largest number of TCs is chosen:
...
Increasing the slice-cutoff by a very small amount greatly increases the number of TCs, by preventing merging of major TSSs by weakly expressed CTSSs. We can then use this tuned cutff value to identify tag cluster
...
Note, we no longer have any very long TCs (For a real dataset it's not uncommon to have a just few very long TCs left). Let's take a closer look at what's returned:
...
This bidirectional clustering approach identifies _any_ bidirectional site in the genome. This means that for example bidirectional promoters will also detected (given that they are sufficiently balanced). To remove these cases, annotation with transcript models can be used to remove bidirectional cluster overlapping known promoters and exons. This is described in the next section.

## Cluster level analysis

Once interesting clusters (unidirectional TSSs and bidirectional enhancers) have been identified, they have to be quantified, annotated with transcript models. We show this functionality using the built-in example data:
...
To look at differential expression, we must quantify the expression of each TSS in each sample (in almost all cases you want to quantify the raw CTSS count). This can be done using the `quantifyClusters` function (The example datasets have already been quantified, so here we will requantify the ranges):

```{r quantifyClusters, tidy=TRUE}
requantified_TSSs <- quantifyClusters(exampleCTSSs, clusters=rowRanges(exampleUnidirectional), inputAssay="counts")
requantified_enhancers <- quantifyClusters(exampleCTSSs, clusters=rowRanges(exampleBidirectional), inputAssay="counts")
```

This returns a RangedSummarizedExperiment with clusters in rowRanges and an expression matrix in assays.

### Removing weakly expressed clusters

We often wish to remove weakly expressed features. Similarly to CTSSs, a simple approach is to remove cluster based on counts using the `subsetBySupport` function:

```{r supportOnCounts, tidy=TRUE}
# Only keep enhancers expressed in more than one sample
supported_enhancers <- subsetBySupport(exampleBidirectional, inputAssay="counts", unexpressed=0, minSamples=1)
```
...
exampleUnidirectional <- subsetBySupport(exampleUnidirectional, inputAssay="TPM", unexpressed=1, minSamples=2)
```
...
- Use the `TxDb` included as Bioconductor packages (**INSERT LINK**): For this vignette we use the `r Biocpkg("TxDb.Mmusculus.UCSC.mm9.knownGene")` package
...
Note, that a single TSS can overlap multiple transcripts (names are separated with a `;`):
...
exampleUnidirectional <- assignTxType(exampleUnidirectional, txModels=txdb, outputColumn="txTyp")
```

The function returns a small summary of the annotation: At the top of the hierachy we have overlap with annotated promoter (+/- 100 basepairs from the annotated TSSs, with this distance being changeable by the user.), followed by proximal (- 1000 upstrema of the annotated TSS). Coding transcripts can be annotated as 5'-UTR, 3-'UTR, CDS and intron, while non-conding transcripts are simply annotated as exon or intron. At the bottom of the hierachy, clusters overlapping genes on the opposite strand are annotated as antisense. Finally, in case a cluster does not overlap any transcript, it is annotated as intergenic.
...
exampleUnidirectional <- assignTxType(exampleUnidirectional, txModels=txdb, outputColumn="peakTxType", swap="thick")
```

Annotation with transcript models are particular useful for enhancers, since bidirectional cluster might in some cases be very balanced bidirectional promoters. We can remove enhancers overlapping known transcripts:
...
# Calculate shape
exampleUnidirectional <- calcShape(exampleUnidirectional, pooled=exampleCTSSs, outputColumn = "IQR", shapeFunction = shapeIQR, lower=0.25, upper=0.75)
```
...
`shapeEntropy` and `shapeMultimodality` works in the same way. Advanced users may implement their own shape statistic by writing a new function:
...
# Calculate lean statistics, additional arguments can be passed to calcShape via "..."
...
CAGE can detect TSSs and enhancers completely independent of existing gene models. However many databases of functional annotation, such as GO and KEGG terms, are only available at gene level. Similarly, one might need to compare CAGE data to existing RNA-Seq or Microarray data only available at the gene level.
...
In a similar fashion to transcript annotation described in 5.2.3, gene models are obtained via `TxDb` objects:

```{r geneSetup, tidy=TRUE}
# Load example TSS 
data("exampleUnidirectional")

# Remove weakly expressed features
exampleUnidirectional <- calcTPM(exampleUnidirectional, totalTags = "totalTags")
exampleUnidirectional <- subsetBySupport(exampleUnidirectional, inputAssay="TPM", unexpressed=1, minSamples=2)
...
TSSs not overlappping any genes recieving NA, and in the (usually rare) case of a TSS overlapping multiple genes, the gene with the closest annotated TSS is chosen:
...
symbols <- mapIds(odb, keys=rowRanges(exampleUnidirectional)$geneID, 
...
For most gene-level operations in `CAGEfightR` NAs (TSSs not overlapping genes) are simply renmoved. If you wish to keep these TSSs, you can relabel them as "Novel" genes via the `assignMissingID` function:

```{r assignMissing, tidy=TRUE}
exampleUnidirectional <- assignMissingID(exampleUnidirectional, outputColumn="symbol")
```

### Quantify expression as gene level

Once TSSs have been assigned to genes, TSSs expression can be summed up within genes to obtain a gene-level expression matrix via the `quantifyGenes` function:
...
### Removing TSSs based on gene composition

When looking at Differential TSS Usage (DTU) we are interested in changes in the contribution of individual TSSs to overall gene expression. Since CAGE can detect even very lowly expressed TSSs, it is common to see TSSs making up only a very small fraction of the total gene expression. Removing these TSSs can improve power and clarity of downstream analysis.

`CAGEfightR` can calculate the _composition_ value of intragenic TSSs, defined as the number of samples expressing a TSS as more than a certain fraction of total gene expression. This is implemented in the `calcComposition` function:
...
The `subsetByComposition` function wraps the common task of calling `calcComposition` and `subset`. After filtering, the resulting expression matrix can be used with popular Bioconductor packages for DTU calling, such as DEXSeq, DRIMSeq, edgeR and limma.
...
Cluster decomposition, sample-wise shapes, DelayedArrays, etc...
