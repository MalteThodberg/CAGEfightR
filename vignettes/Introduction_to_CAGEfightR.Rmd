...
# Specify the number of cores to use (for this small example just 1)
...
```{r quickGenes, tidy=FALSE}
gene_level <- quickGenes(RSE, geneModels = txdb)
```

But it is much cooler to leverage some of the unique features 5'-end methods, for example, we can calculate the correlation of expression of nearby TSSs and enhancers:
...
Other S4-classes includes `TxDb`-objects from `r BiocStyle::Biocpkg("GenomicFeatures")`, `GInteractions`-object from `r BiocStyle::Biocpkg("InteractionSet")` and various track-objects from `r BiocStyle::Biocpkg("Gviz")`. See [Annotation with transcript models], [Spatial analysis of clusters] and [Plotting CAGE data in a genome browser] for more details.
...
- `check*`-functions for making sure object are correctly formatted.
- `convert*`-function converts CTSSs between different file formats.
...
- `utils*` functions exposes some of the inner workings of `CAGEfightR` and are mainly intended for expert users and developers.
...
The following section contains a detailed walkthrough of most of the functions in `CAGEfightR`, using the built-in dataset. To keep this vignette compact, the example dataset is extremely small - for a more realistic analysis of a full CAGE dataset using both `CAGEfightR` and additional packages, see `r BiocStyle::Biocpkg("CAGEWorkflow")`.
