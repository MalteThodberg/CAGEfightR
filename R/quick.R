...
quickTSSs <- function(object){
	...
	if(is.null(score(rowRanges(object)))){
		...
		object <- calcTPM(object); invisible(gc())
		object <- calcPooled(object); invisible(gc())
	}else{
...
quickEnhancers <- function(object){
	...
	message("\n - Running clusterUnidirectionally:")
	enhancers <- clusterBidirectionally(object); invisible(gc())

	message("\n - Running subsetByBidirectionality:")
	enhancers <- subsetByBidirectionality(enhancers, object); invisible(gc())

	message("\n - Running quantifyClusters:")
	enhancers <- quantifyClusters(object, enhancers); invisible(gc())
