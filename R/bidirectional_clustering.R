...
coverageWindows <- function(ctssCoverage, window, balanceFun, biocParallel=bpparam(), ...){
	# Checks
	stopifnot(window %% 2 == 1, window >= 3)
	stopifnot(is.function(balanceFun)|is.null(balanceFun))
	...
	if(!is.null(balanceFun)){
		...
		# Calculate balance - don't use bpmapply when serial, saves lots of memory
		if(bpworkers(biocParallel) == 1){
			B <- balanceFun(plusUpstream=PU, plusDownstream=PD, minusUpstream=MU, minusDownstream=MD, ...)
		}else{
			endo <- unique(c(class(PU), class(PD), class(MU), class(MD)))
			B <- bpmapply(FUN=balanceFun, PU, PD, MU, MD, BPPARAM=biocParallel, MoreArgs = list(...))
			B <- methods::as(B, endo)
		}
...
#'
#' @param plusUpstream RleList, Rle or other: Sum of plus strand upstream (reference=0.0)
#' @param plusDownstream RleList, Rle or other: Sum of plus strand downstream (reference=0.5)
#' @param minusUpstream RleList, Rle or other: Sum of minus strand upstream (reference=0.0)
#' @param minusDownstream RleList, Rle or other: Sum of minus strand downstream (reference=0.5)
#'
...
#' @export
bidirectionalClustering <- function(ctssCoverage, window=199, balanceThreshold=0.99, balanceFun=BC, advancedStats=FALSE, biocParallel=bpparam(), ...){
	# Get windows
	...
	if(!sum(elementNROWS(bidirLoci)) > 0){
		stop("No bidirectional sites found at the given balance threshold!")
	}
...
calculateBidirectionality <- function(ctss, features, biocParallel=bpparam()){
	...
	# Quantify
	message(sprintf("Quantifying plus-arm expression of %d features in %d samples...",
	...
	message(sprintf("Quantifying minus-arm expression of %d features in %d samples...",
									length(features), length(ctss)))
...
### UPDATE TO APPLY BC UNDER BPLAPPLY
...
#' }
#'
#' #' Bidirectional clustering of genome-wide coverage FASTER
#' #'
#' #' Finds sites with (balanced and divergent) bidirectional transcription using sliding windows of summed coverage. Custom functions for calculating balance can be used.
#' #'
#' #' @param ctssCoverage GRanges: CTSS coverage.
...
#' #' @import S4Vectors IRanges GenomicRanges
#' #' @export
#' bidirectionalClusteringFaster <- function(ctssCoverage, window=199, balanceThreshold=0.99, balanceFun=BC, advancedStats=FALSE, biocParallel=bpparam(), ...){
#' 	# Checks
#' 	stopifnot(window %% 2 == 1, window >= 3)
#' 	stopifnot(is.function(balanceFun))
#'
#' 	# Obtain shift
#' 	shift_val <- ceiling(window / 2)
#'
...
#' 	# Calculate balance
#' 	endo <- unique(c(class(PU), class(PD), class(MU), class(MD)))
#' 	B <- bpmapply(FUN=BC, PU, PD, MU, MD, BPPARAM=biocParallel)
#' 	B <- methods::as(B, endo)
#' 	#rm(PD, PU, MD, MU)
#'
#' 	# Set NAs to 0 to allow slicing
...
#' # # axisTrack <- GenomeAxisTrack(littleTicks=TRUE, exponent=0)
#' # # covTrack <- CAGEfightR::strandedCoverageTrack(gr=ctssCoverage, name="TPM")
#' # balanceTrack <- DataTrack(GRanges(B), name="Balance", type="l", col="forestgreen")
