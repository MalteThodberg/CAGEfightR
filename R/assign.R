...
setMethod("assignTxType", signature(object = "GenomicRanges", txModels = "GenomicRangesList"), 
    function(object, txModels, outputColumn = "txType", swap = NULL, noOverlap = "intergenic") {
        # Pre-checks
        assert_that(!is.null(names(txModels)), is.string(outputColumn), is.string(noOverlap), 
...
#' @rdname assignTxType
setMethod("assignTxType", signature(object = "RangedSummarizedExperiment", txModels = "GenomicRangesList"), 
...
setMethod("assignTxType", signature(object = "GenomicRanges", txModels = "TxDb"), 
    function(object, txModels, outputColumn = "txType", swap = NULL, tssUpstream = 100, 
        tssDownstream = 100, proximalUpstream = 1000, detailedAntisense = FALSE) {
        # Pre-checks
        assert_that(is.count(tssUpstream), is.count(tssDownstream), is.count(proximalUpstream), 
            is.flag(detailedAntisense), identical(seqlengths(object), seqlengths(txModels)))
        ...
        # Overlap
        object <- assignTxType(object = object, txModels = hierachy, outputColumn = outputColumn, 
            swap = swap, noOverlap = "intergenic")
...
setMethod("assignGeneID", signature(object = "GenomicRanges", geneModels = "GenomicRanges"), 
    function(object, geneModels, outputColumn = "geneID", swap = NULL, upstream = 1000, 
        downstream = 100) {
        # Pre-checks
        assert_that(!is.null(names(geneModels)), is.string(outputColumn), is.number(upstream), 
            is.number(downstream), identical(seqlengths(object), seqlengths(geneModels)))
        ...
        extendedGeneModels <- punion(geneModels, promoters(geneModels, upstream = upstream, 
...
setMethod("assignGeneID", signature(object = "GenomicRanges", geneModels = "TxDb"), 
    function(object, geneModels, outputColumn = "geneID", swap = NULL, upstream = 1000, 
        downstream = 100) {
        # Pre-checks
        assert_that(is.string(outputColumn), is.number(upstream), is.number(downstream), 
...
setMethod("assignTxID", signature(object = "GenomicRanges", txModels = "GenomicRanges"), 
    function(object, txModels, outputColumn = "txID", swap = NULL) {
        # Pre-checks
        assert_that(!is.null(names(txModels)), is.string(outputColumn), identical(seqlengths(object), 
            seqlengths(txModels)))
        ...
        # Return
        message("### Overlap Summary: ###")
        message("Features overlapping transcripts: ", round(mean(!is.na(hits)) * 
            100, digits = 2), " %")
...
#' @rdname assignTxID
setMethod("assignTxID", signature(object = "GenomicRanges", txModels = "TxDb"), function(object, 
    txModels, outputColumn = "txID", swap = NULL, upstream = 1000, downstream = 0) {
    # Pre-checks
    assert_that(is.string(outputColumn), is.number(upstream), upstream >= 0, is.number(downstream), 
        downstream >= 0, identical(seqlengths(object), seqlengths(txModels)))
...
setMethod("assignMissingID", signature(object = "GenomicRanges"), function(object, 
    ...
    # Pre-checks
    assert_that(is.string(outputColumn), outputColumn %in% colnames(mcols(object)))
